...
Cài các package được management scripts và chức năng DNS per-VM của
Proxy Gateway v1.0.3 sử dụng.
...
Proxy Gateway không sử dụng Unbound mặc định của hệ thống. Mỗi VM được
chạy bằng một Unbound instance riêng thông qua
`proxy-gateway-dns@<INSTANCE>.service`.

Tắt default Unbound service nếu package vừa cài đã tự khởi động nó:
...
Mỗi lệnh phải trả về một executable path hợp lệ.

Kiểm tra default Unbound không chạy:
...
> `unbound` là dependency bắt buộc của tính năng DNS leak protection
> trong v1.0.3. Nếu thiếu package này, Web UI sẽ báo
> `ERROR: unbound is not installed.` khi Add VM.
...
Proxy Gateway v1.0.3 sử dụng ba systemd unit files:
...
`hev-socks5-tunnel@.service` là template service cho HEV tunnel của từng VM.

`proxy-gateway-dns@.service` là template service cho Unbound DNS riêng
của từng VM và là một phần của cơ chế DNS fail-close.

`proxy-gateway-ui.service` chạy Flask Web UI thông qua Gunicorn.
...
## 8.3 Install the Per-VM DNS Template Service

Cài systemd template cho Unbound DNS riêng của từng VM:
...
Yêu cầu systemd đọc lại unit vừa cài:
...
Không start trực tiếp `proxy-gateway-dns@101.service` ở bước này.
Service instance chỉ được tạo/start sau khi Add VM đã sinh file:


```text
/etc/unbound/proxy-gateway/vm101.conf
```


Ví dụ các instance sau này:


```text
proxy-gateway-dns@101.service
proxy-gateway-dns@102.service
proxy-gateway-dns@104.service
```


---

## 8.4 HEV Service Lifecycle
...
## 8.5 Install the Web UI Service
...
## 8.8 Managing HEV Instances

> **Mục này chỉ dùng để tham khảo.**
>
> Không start/stop/restart một HEV instance trong quá trình cài đặt ban đầu.
> Web UI/Add VM sẽ tạo cấu hình và enable service instance tương ứng.
...
'hev-socks5-tunnel|proxy-gateway-dns|proxy-gateway-ui'
```
...
Có thể kiểm tra trực tiếp cả ba unit:
...
Ở thời điểm này chưa cần có `proxy-gateway-dns@101.service` đang active,
vì VM101 chưa được Add.
...
Các management script được cài với owner `root:root` và mode `750`.
Vì vậy phải kiểm tra executable permission với quyền root; kiểm tra bằng
`[ -x "$file" ]` từ user thường có thể báo sai rằng file không executable.
...
for file in \
    ...
    if sudo test -x "$file"; then
        ...
        echo "MISSING OR NOT EXECUTABLE BY ROOT: $file"
    fi
done
```


Cả 7 file phải báo `OK`.

Không đổi permission sang `755`; mode `750 root:root` là thiết kế chủ ý
cho management scripts.
...
Cả ba unit files phải load thành công.
